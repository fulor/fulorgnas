...
if ( ! class_exists( 'OT_Meta_Box' ) ) {
  ...
  class OT_Meta_Box {
    ...
    function build_meta_box( $post, $metabox ) {
      ...
      echo '<div class="ot-metabox-wrapper">';
        ...
        foreach ( $this->meta_box['fields'] as $field ) {
          ...
          $field_value = ( 'picture' == $post->post_type ) ? get_picture_meta( $post->ID, $field['id'], true ) : get_post_meta( $post->ID, $field['id'], true );
    ...
    function save_meta_box( $post_id, $post_object ) {
      ...
      /* save picture meta in separate table */
      if ( 'picture' == $post_object->post_type ) {
      	$this->save_picture_meta_box( $post_id, $post_object );
      	return;
      }
     ...
     * @since     1.0
     */
    function save_picture_meta_box( $post_id, $post_object ) {
      global $pagenow;

      foreach ( $this->meta_box['fields'] as $field ) {

        $old = get_picture_meta( $post_id, $field['id'], true );
        $new = '';
        ...
        if ( $new && $new !== $old ) {
          update_picture_meta( $post_id, $field['id'], $new );
        } else if ( '' == $new && $old ) {
          delete_picture_meta( $post_id, $field['id'], $old );
        }
      }
  
	} // Picture meta box save 
